...
for L in $src $tgt; do
    for split in train valid test; do
        ...
        echo "Unique tokens in $split.$L improved BPE: $unique_token_count" >> unique_token_counts_improved_bpe.log
    done
done

echo "Unique token counts in improved BPE saved to unique_token_counts_improved_bpe.log."
...
mkdir -p checkpoints/trsfm_new_improved_bpe
...
CUDA_VISIBLE_DEVICES=0 fairseq-train data-bin/iwslt13.tokenized.fr-en \
    ...
    --max-tokens 4000 \
    --save-dir checkpoints/trsfm_new_improved_bpe \
    --max-epoch 10

# Evaluate Transformer Model
echo "Evaluating Transformer model..."
fairseq-generate data-bin/iwslt13.tokenized.fr-en \
    --path checkpoints/trsfm_new_improved_bpe/checkpoint_best.pt \
    --batch-size 128 \
    --beam 5 \
    --remove-bpe \
     --scoring sacrebleu > translation_result/generate_trsfm_improved_bpe.log
